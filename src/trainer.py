...
def calc_diff(C_pre, C_pos, t_pre, t_pos, rss_pre, rss_pos):
    ...
    ratio_t = np.mean(
        np.linalg.norm(t_pos - t_pre, axis=1) / np.linalg.norm(t_pre, axis=1)
    )
    ...
    def train(
        ...
        for itr in range(max_iteration):
            self.bezier_simplex.write_control_point(
                C=C_pre,
                filename=result_dir
                + "/control_points/control_point_itr_"
                + "{0:03d}".format(itr),
            )
        ...
        self.bezier_simplex.write_control_point(
            C_pos,
            result_dir
            + "/control_points/control_points_itr_"
            + "{0:03d}".format(itr + 1),
        )
        self.bezier_simplex.write_control_point(
            C_pos, result_dir + "/control_points_itr" + "{0:03d}".format(itr + 1)
        )
...
class InductiveSkeletonTrainer:
    ...
    def insert_subproblem_control_point_to_whole_contol_point(
    ...
    def train(
            ...
            for index in data:
                if len(index) == dim:
                    ...
                    if len(freeze_multiple_index_set) == len(C.keys()):
                        ...
                        target_multiple_index_set = self.extract_corresponding_multiple_index(
                            C=C, index_list=index
                        )
                        if dim >= 2:
                            ...
                            freeze_ = self.extract_corresponding_multiple_index_from__set(
                                s=freeze_multiple_index_set, index_list=index
                            )
                            ...
                            C = self.insert_subproblem_control_point_to_whole_contol_point(
                                C_whole=C, C_sub=C_sub, index_list=index
                            )
                        freeze_multiple_index_set = freeze_multiple_index_set.union(
        ...
        self.write_control_point(
            C=C, filename=result_dir + "/control_points_itr_" + "{0:03d}".format(loop)
        )
            ...
            for index in data_dict:
                if len(index) == dim:
                    target_multiple_degree_set = subfunction.extract_multiple_degree(
                        degree_list=indices_all, index_list=index
                    )
                    ...
                    control_point.update(a)
                    freeze_multiple_degree_set = freeze_multiple_degree_set.union(
                        target_multiple_degree_set
                    )
                    if len(freeze_multiple_degree_set) == len(indices_all):
                        break
...
if __name__ == "__main__":
    ...
    print("initialize ")
    tt, xx = borges_pastva_trainer.initialize_parameter(
        c=C_init, data=data[(1, 2, 3, 4, 5)]
    )
    print(tt.shape, xx.shape)
    print("inner product")
    f = borges_pastva_trainer.inner_product(
        c=C_init, t=tt[2, :], x=data[(1, 2, 3, 4, 5)][2, :]
    )
    print(f)
    ...
    print("jaccobian ")
    j = borges_pastva_trainer.inner_product_jaccobian(
        c=C_init, t=tt[2, :], x=data[(1, 2, 3, 4, 5)][2, :]
    )
    print(j)

    print("update parameter")
    tt_, xx_ = borges_pastva_trainer.update_parameter(
        c=C_init, t_mat=tt, data=data[(1, 2, 3, 4, 5)]
    )
    ...
    C_ = borges_pastva_trainer.update_control_point(
        t_mat=tt_,
        data=data[(1, 2, 3, 4, 5)],
        c=C_init,
    ...
    C_k = borges_pastva_trainer.train(
        data=data[(1, 2, 3, 4, 5)],
        C_init=C_init,
        max_iteration=30,
        tolerance=10 ** (-4),
