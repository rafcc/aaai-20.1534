...
def experiments_practical_instances(data_dir,trn_data,test_data,n,solution_type,dimsimplex,degree,method,seed,results_dir,opt_flag=1):
    """
    ...
    for e in subproblem_indices_list:
        if len(e) <= degree or len(e) == dimsimplex:
            ...
            tmp = data.Dataset(data_dir+'/'+trn_data+','+solution_type+'_'+string)
    ...
    if method == "borges":
        ...
        borges_pastva_trainer = trainer.BorgesPastvaTrainer(dimSpace=dimspace,
                                                            ...
                                                            degree = degree)
        control_point = borges_pastva_trainer.update_control_point(t_mat = param_trn[e],
                                                                   data=data_trn[e],
                                                                   c = {},
    ...
    e = tuple([i for i in range(dimsimplex)])
    data_tst = data.Dataset(data_dir+'/'+test_data+','+solution_type+'_'+'_'.join([str(i+1) for i in e])).values
    param_tst = data.Dataset(data_dir+'/'+test_data+',w_'+'_'.join([str(i+1) for i in e])).values
        ...
        for solution_type in solution_type_list:
            for n in n_list:
                for seed in seed_list:
                    ...
                    experiments_practical_instances(data_dir="../data",
                                                    trn_data="MED,d_100,o_3,c_2e+00,e_1e-01,n_10000,s_42",
                                                    test_data="MED,d_100,o_3,c_2e+00,e_1e-01,n_10000,s_43",
                                                    solution_type=solution_type,
                                                    n=n,degree=degree,dimsimplex=3,seed=seed,method="inductive",opt_flag=1,
                                                    results_dir=results_dir)
                    experiments_practical_instances(data_dir="../data",
                                                    trn_data="MED,d_100,o_3,c_2e+00,e_1e-01,n_10000,s_42",
                                                    test_data="MED,d_100,o_3,c_2e+00,e_1e-01,n_10000,s_43",
                                                    solution_type=solution_type,
                                                    n=n,degree=degree,dimsimplex=3,seed=seed,method="borges",opt_flag=1,
                                                    results_dir=results_dir)
