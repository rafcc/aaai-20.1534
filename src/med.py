...
class Med(object):
    ...
    def f(self, i, x):
        return (
            np.linalg.norm((self.A.dot(x) - self.e[i]) / np.sqrt(2), ord=2)
            ** self.convexity
        )
...
@click.option(
    "-c", "--convexity", type=float, required=True, help="Front shape parameter"
)
@click.option(
    "-e",
    "--noise",
...
def main(subproblem, dim, obj, convexity, noise, size, seed):
    ...
    weights = runif_simplex(n=size, dim=obj, face=[i - 1 for i in subproblem])
    ...
    with click.progressbar(weights, show_pos=True, item_show_func=isf) as bar:
        decisions = [
            x_star(w, med) + np.random.normal(scale=noise, size=med.dim()) for w in bar
        ]
    objectives = [med(x) for x in decisions]
    s = "_".join(str(i) for i in subproblem)
    fn = (
        "MED,d_{},o_{},c_{:1.0e},e_{:1.0e},n_{},s_{}".format(
            dim, obj, convexity, noise, size, seed
        )
        + ",{}_"
        + s
    )
